...
const Overview = ({ providerDescription: description, eligibility, requirements }) => {
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <section>
        ...
        <div className="space-y-4 text-slate-600 leading-relaxed font-medium">
          {/* Description could be an array or string */}
        ...
        <div className="space-y-6">
          {requirements?.map((item, idx) => (
            <RequirementItem
              key={idx}
              title={item.name}
              desc={Array.isArray(item.description) ? item.description.join(', ') : item.description}
            />
          ))}
        </div>
      </section>
...
const RequirementItem = ({ title, desc }) => (
  <div className="flex items-start gap-4">
    <div className="w-12 h-12 rounded-full bg-[#137fec]/5 border border-[#137fec]/10 flex items-center justify-center text-[#137fec] shrink-0"></div>
